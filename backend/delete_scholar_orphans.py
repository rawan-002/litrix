"""
Delete the 18 Scholar orphan papers - same safety pattern.

USAGE:
    python delete_scholar_orphans.py --dry-run
    python delete_scholar_orphans.py
"""
...
def child_counts(cur, paper_id, tables):
    counts = {}
    for t in tables:
        cur.execute(f'SELECT COUNT(*) FROM "{t}" WHERE "PaperID" = %s', (paper_id,))
...
def main():
    ...
    with transaction.atomic():
        with connection.cursor() as cur:
            ...
            print(f"\nScholar orphans to delete: {len(targets)}")
            if not targets:
                print("Nothing to do.")
                return
            ...
            print("\nSample (first 3):")
            for t in targets[:3]:
                print(f"  PaperID={t[0]:<6} year={t[2]}  doi={t[3]}  scholar={t[5]}")
                print(f"    title: {(t[1] or '')[:90]}")

            n_deleted = 0
            for paper_id, title, pub_year, doi, source, scholar_id in targets:
                ...
                snapshot = {
                    ...
                    "DOI":      doi,
                    "Source":   source,
                    "ScrapedFromScholarID": scholar_id,
                    "ChildCountsBefore": counts,
                    "Reason":   "scholar_orphan_user_removed",
                }
                cur.execute(
                    ...
                    ["paper.delete.scholar_orphan", "ResearchPaper", paper_id,
                     json.dumps(snapshot),
                     "backfill_script:delete_scholar_orphans"]
                )
                for t in tables:
                    cur.execute(f'DELETE FROM "{t}" WHERE "PaperID" = %s', (paper_id,))
                cur.execute('DELETE FROM "ResearchPaper" WHERE "PaperID" = %s', (paper_id,))
                n_deleted += 1
            ...
            if args.dry_run:
                ...
                print("Recovery: AuditLog WHERE Action='paper.delete.scholar_orphan'")
